...
const doubled = myNumbers.map((element, index, array) => {
	return element*2
})
...
function greaterThanBenjamin(element, index, wholeArray){
	if(element > 100) {
		return element;
	}
}

 const filtered = myNumbers.filter(greaterThanBenjamin)
...
let forTheLoveOfGeorge = notGeorge.map((element, index, arr) => {
...
for(friend in people){
	friend.filter(people.friend === false)
}
//////////////////PROBLEM 20////////////////////
...
const totallyAwesome = people.awesomeLevel.reduce(acc, curr){
 	acc += curr
 }


// const productOfArray = () => numbers.reduce((acc, curr)=>acc*=curr)
// numbers = [1,3,5];
